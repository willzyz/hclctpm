...
num_optimize_iterations = 500 ## number of optimization iterations 
...
with gs_tqr[best_index].as_default() as g: 
    h_test = forwardCTPMMatcherD2DistDNN(Da, Db, int, num_hidden) 
...
with gs_drm[best_index].as_default() as g: 
    h_test = forwardSimpleTCModelDNN(D, num_hidden) 
...
pred_values_va_rlearner = pred_values_va_rlearner_O * (pred_values_va_rlearner_D - d2dlamb * pred_values_va_rlearner_C) 
...
exp = Experimentation() 
ranscore = np.random.rand(o.shape[0], ) 
colors = ['b', 'g', 'y'] 
plt.figure() 
rlearnerauccs = [] 
...
ranaucc = exp.AUC_cpit_cost_curve_deciles_cohort_d2dist(ranscore, o, w, -1.0 * c, d2d, d2dlamb, 'k', plot_random=True) 
#quasiaucc = exp.AUC_cpit_cost_curve_deciles_cohort_d2dist(pred_values_va_rlearner_O, ot, wt, -1.0 * ct, d2dt, d2dlamb, 'c') 
#for i in range(len(lambds)): 
#    rlearnerauccs.append(exp.AUC_cpit_cost_curve_deciles_cohort_d2dist(rlearnerscores[i], ot, wt, -1.0 * ct, d2dt, d2dlamb, colors[i] )) 
#cfaucc = exp.AUC_cpit_cost_curve_deciles_cohort(cfscore, ot, wt, -1.0 * ct, 'g') # causal forest aucc and plotting 
ctpmaucc = exp.AUC_cpit_cost_curve_deciles_cohort_d2dist(ctpmscore, o, w, -1.0 * c, d2d, d2dlamb, 'r' ) 
drmaucc = exp.AUC_cpit_cost_curve_deciles_cohort_d2dist(drmscore, o, w, -1.0 * c, d2d, d2dlamb, 'm' ) 
