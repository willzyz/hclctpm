...
prefix = 'uscensus_pub_causal_data'
num_trees = 50 
p_alpha = 0.2 #0.05 
...
dread <- read.csv(file=paste('data/causal_forest_r_data_', prefix, '_train.csv', sep='')) 
...
dread <- read.csv(file=paste('data/causal_forest_r_data_', prefix,'_test.csv', sep='')) 
...
write.csv(tdfO, file = paste('results/causal_forest_grf_test_set_results_O_',prefix, '_numtrees', toString(num_trees), '_alpha', toString(p_alpha), '_min_node_size', toString(p_min_node_size), '_sample_fraction', toString(p_sample_fraction),'.csv', sep='')) 

print('predicting and writing to csv, cost') 
tdfC <- as.data.frame(t(tauC.hat)) 
write.csv(tdfC, file = paste('results/causal_forest_grf_test_set_results_C_',prefix, '_numtrees', toString(num_trees), '_alpha', toString(p_alpha), '_min_node_size', toString(p_min_node_size), '_sample_fraction', toString(p_sample_fraction),'.csv', sep='')) 
